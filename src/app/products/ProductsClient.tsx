...
import { allProductsWithG1, type Product, type GradeKey } from '@/lib/products'

const CATS = [
  { key:'all',         label:'All Products',              count: 0 },
  { key:'electronic',  label:'Electronic Grade Solvents', count: 0 },
  { key:'g1',         label:'G1 Grade (Entry)',           count: 0 },
...
const ALL_GRADES: GradeKey[] = [
  'G1','G2','G3','G4','G5','PHARMA','HPLC_PREP','HPLC_GRADIENT',
...
export default function ProductsClient() {
  ...
  const filtered = useMemo(() => {
    let list = cat === 'all' ? allProductsWithG1 : cat === 'g1' ? allProductsWithG1.filter(p => p.grades.includes('G1')) : allProductsWithG1.filter(p => p.category === cat)
      ...
      <section className="py-10">
        <Container>
          ...
          <div className="flex gap-2 flex-wrap mb-3">
            {CATS.map(c => {
              const count = c.key === 'all' ? allProductsWithG1.length : allProductsWithG1.filter(p => p.category === c.key).length
              return (
