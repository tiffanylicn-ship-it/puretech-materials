...
import { allProducts, allProductsWithG1, ALL_CATEGORY_META, GRADE_META, type GradeKey } from '@/lib/products'
import { GradeCompareChart } from '@/components/ui/GradeChart'

export function generateStaticParams() {
  return allProductsWithG1.map(p => ({ id: p.id }))
}

export async function generateMetadata({ params }:{ params:{ id:string } }): Promise<Metadata> {
  const p = allProductsWithG1.find(x => x.id === params.id)
...
export default function ProductDetailPage({ params }:{ params:{ id:string } }) {
  ...
  const related    = allProductsWithG1.filter(r => r.category===p.category && r.id!==p.id).slice(0,4)
  ...
  const hasG1     = p.specs.some(r=>r.g1)
  ...
  return (
    <>
                    ...
                    <tbody>
                      {p.specs.map((row,i)=>(
                        <tr key={i}>
                          ...
                          {hasG1    && <td><span className="font-mono text-[12.5px] font-medium text-[#506880]">{row.g1     || '—'}</span></td>}
      ...
      <section className="py-14">
        <Container>
          <Reveal className="mb-8">
            <Eyebrow>Grade Comparison</Eyebrow>
            <h2 className="font-serif text-[26px] text-[#0A1628] tracking-[-0.3px]">G1 → G5 Purity Scale</h2>
            <p className="text-[14px] text-[#2C4160] mt-2 max-w-[600px] leading-[1.65]">
